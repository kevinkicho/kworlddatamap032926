...
// ── Module state ──────────────────────────────────────────────────────────────
// (map moved to state.js)
// (wikiLayer moved to state.js)
// (rawCities moved to state.js)
// (allCities moved to state.js)
// (cityByQid moved to state.js)
// (countryData moved to state.js)
// (_avgScoreCache moved to state.js)
// (_rankCacheByRegion moved to state.js)
// (filtered moved to state.js)
// (visibleCount moved to state.js)
const PAGE_SIZE = 100;
// (sortCol moved to state.js)
// (sortDir moved to state.js)
// (editingKey moved to state.js)

// Companies data keyed by city QID
// (companiesData moved to state.js)
// (companiesDetailData moved to state.js)
/**
...
// Maps URL stems to kdb namespace keys. Lite variants map to the full version.
const _URL_TO_KDB = {
...
function _kdbGet(stem) {
  ...
  var val = S.kdb[key];
  if (!val) return null;
  // For lite variant requests on full data, extract the lite-relevant subset
  if (_URL_TO_KDB[stem] && _URL_TO_KDB[stem] !== stem) {
    var full = val;
    if (full && typeof full === 'object') {
      // Lite variant pattern: full data has { fetched_at, source, ..., byCountry, incidents, etc. }
      // The lite version typically has a subset. Return a simplified version.
      // For most lite variants, the full data works as-is since the toggle code
      // accesses the same properties (e.g. .flights, .satellites, .byCountry)
      // The lite just had fewer entries; the full version is a superset.
    }
  }
  // For admin1 per-country, stem is 'admin1/XX' → kdb.admin1.XX
...
// Returns a Response-like object: { ok, json() } if kdb hit, else real fetch Response.
// For kdb hits, the data is already parsed so .json() returns it directly.
...
function applyMapFilters(city) {
  ...
  if (S._filterValue.pop !== null) {
    ...
    if (typeof S._filterValue.pop === 'object') {
      ...
      // Handle legacy bucket filters
      if (S._filterValue.pop === 'small'  && p >= 100_000)                     return 'other';
      if (S._filterValue.pop === 'medium' && (p < 100_000  || p >= 1_000_000)) return 'other';
      if (S._filterValue.pop === 'large'  && (p < 1_000_000 || p >= 10_000_000)) return 'other';
      if (S._filterValue.pop === 'mega'   && p < 10_000_000)                   return 'other';
    }
  }
...
function _filterCount() {
  return Object.values(S._filterAvail).filter(Boolean).length +
         Object.values(S._filterValue).filter(v => v !== null).length;
}
...
function _valueFilterColor(city) {
  ...
  if (S._filterValue.nobel != null) {
    ...
    if (n < S._filterValue.nobel) return null; // not matched
...
function clearAllFilters() {
  ...
  Object.keys(S._filterAvail).forEach(k => { S._filterAvail[k] = false; });
  Object.keys(S._filterValue).forEach(k => { S._filterValue[k] = null; });
...
async function toggleAvailFilter(key) {
  ...
  // If cities are hidden, turn them on first so filters are visible
...
// Slider uses log scale: 3 = 1k, 4 = 10k, 5 = 100k, 6 = 1M, 7 = 10M, 8 = 100M
const POP_SCALE = { min: 3, max: 8 };
...
function _updatePopSliderUI(minVal, maxVal) {
  ...
  // Check if showing "Any" (full range)
...
function setPopRange(minVal, maxVal) {
  // Store as population values (log scale)
  ...
  if (isAny) {
    ...
    // Store as object for range filtering
...
function clearHeatmap() {
  ...
  if (S._heatmapLayer) { S._heatmapLayer.remove(); S._heatmapLayer = null; }
  const palRow  = document.getElementById('heat-palette-row');
...
function _passesMetricValueFilter(metric, city) {
  ...
  switch (metric) {
    case 'nobel':        return (S.nobelCitiesData[qid]?.total   ?? 0) >= fv;
    case 'universities': return (S.universitiesData[qid]?.length ?? 0) >= fv;
    case 'metro':        return (S.metroTransitData[qid]?.stations ?? 0) >= fv;
    case 'aq':           return (S.airQualityData[qid]?.category ?? '') === fv;
    case 'pop': {
      const p = city.pop || 0;
...
function _buildHeatPoints(metric) {
  ...
  S._heatNormP95[metric] = p95;   // cache for _metricDotColor
...
function _refreshHeatLayer() {
  ...
  if (points.length > 0 && typeof L !== 'undefined' && L.heatLayer) {
    S._heatmapLayer = L.heatLayer(points, {
      radius: S._heatRadius, blur: S._heatBlur, maxZoom: 10, max: 1.0,
...
function _updateAvailIntensityStrips() {
  document.querySelectorAll('#filter-avail-list .filter-avail-row').forEach(row => {
    ...
    if (isOn && metric) {
      strip.style.background = S._matchedColorMode === 'pop'
...
function _computeP95(metric) {
  ...
  if (!vals.length) { S._heatNormP95[metric] = 1; return; }
  vals.sort((a, b) => a - b);
...
// Choropleth state
// (choroplethLayer moved to state.js)
// (_choroHoverTarget moved to state.js)
// (_choroLayerByIso moved to state.js)
// (worldGeo moved to state.js)
// (choroOn moved to state.js)
// (activeChoroKey moved to state.js)
// (econLayer moved to state.js)
// (econOn moved to state.js)
// (tradeArrowLayer moved to state.js)
const tradeCache     = {};     // iso2 → [{year, expGds, impGds}]
const countryCentroids = {};   // iso2 → [lat, lng]

// Admin-1 (subnational boundaries) layer state
// (admin1On moved to state.js)
// (admin1Layers moved to state.js)
// (admin1Index moved to state.js)
const admin1Cache    = {};     // iso2 → TopoJSON object
// (_admin1Loading moved to state.js)
// (_admin1HoverTarget moved to state.js)
// (_admin1HoverIso moved to state.js)
...
// (_editsCache moved to state.js)
function loadEdits() {
  if (S._editsCache !== null) return S._editsCache;
  try { S._editsCache = JSON.parse(localStorage.getItem(LS_EDITS) || '{}'); }
  catch { S._editsCache = {}; }
...
function saveEditsStore(edits) { localStorage.setItem(LS_EDITS, JSON.stringify(edits)); S._editsCache = null; }
...
// (validateCities, cityKey → utils.js)
...
// (popToT, lerpRGB, wikiCityColor, wikiCityOpacity, wikiCityRadius, fmtPop, fmtNum → utils.js)
...
function rebuildMapLayer() {
  ...
  S.allCities.forEach(function (city) {
    ...
    if (S.cityAqMode && city.qid) {
      ...
      if (aq) tip += `<br/><span style="color:${color};font-weight:600">${aq.pm25} µg/m³ PM2.5</span> <span style="color:var(--text-secondary);font-size:0.8em">(${escHtml(aq.category)}, ${aq.year})</span>`;
    ...
    L.circleMarker([city.lat, city.lng], {
      ...
      pane: 'cityPane',
    }).bindPopup(tip, { maxWidth: 260, minWidth: 160 })
      .on('click', () => { if (city.qid) openWikiSidebar(city.qid, city.name); })
      .addTo(S.wikiLayer);
  });
...
function sortFiltered() {
  S.filtered.sort((a, b) => {
    let av, bv;
    if (S.sortCol === 'pop') { av = a.pop ?? -Infinity; bv = b.pop ?? -Infinity; }
    else if (S.sortCol === 'name') { av = a.name ?? ''; bv = b.name ?? ''; }
    else if (S.sortCol === 'country') { av = a.country ?? ''; bv = b.country ?? ''; }
...
// ── Render list rows ──────────────────────────────────────────────────────────
export async function switchListTab(tab) {
...
export function closeGlobalCorpPanel() {
  const corpsPanel = document.getElementById('global-corp-panel');
  if (corpsPanel) { corpsPanel.classList.remove('panel-open'); corpsPanel.style.display = 'none'; }
  document.querySelectorAll('.list-tab').forEach(b => b.classList.remove('active'));
}
...
function renderRows() {
  ...
  document.getElementById('list-meta-text').textContent =
    `Showing ${Math.min(S.visibleCount, S.filtered.length).toLocaleString()} of ${S.filtered.length.toLocaleString()} cities`;
  ...
  if (hasMore) {
    document.getElementById('load-more-btn').textContent =
      `Show more (${(S.filtered.length - S.visibleCount).toLocaleString()} remaining)`;
  }
}
...
function saveEdit() {
  ...
  Object.keys(edits[S.editingKey]).forEach(k => {
    if (edits[S.editingKey][k] === undefined) delete edits[S.editingKey][k];
  });
...
function initHeaderSort() {
  document.querySelectorAll('thead th[data-col]').forEach(th => {
    th.addEventListener('click', function () {
      ...
      else { S.sortCol = col; S.sortDir = (col === 'name' || col === 'country') ? 'asc' : 'desc'; }
...
// ── Stats panel: extracted to src/stats-panel.js ─────────────────────────────

// ── Sidebar tab state ─────────────────────────────────────────────────────────
// (_sidebarTab moved to state.js)
...
// ── Power plant detail view ──────────────────────────────────────────────────
function _buildPowerPlantDetail(p, qid) {
  const fuelIcon = { 'Hydro':'💧', 'Nuclear':'☢️', 'Coal':'⚫', 'Gas':'🔥', 'Oil':'🛢️', 'Solar':'☀️', 'Wind':'🌬️', 'Biomass':'🌿', 'Geothermal':'🌋', 'Waste':'🗑️' };
  const sorted = [...p.plants].sort((a, b) => (b.capacity_mw || 0) - (a.capacity_mw || 0));

  let rows = '';
  for (const plant of sorted) {
    const icon = fuelIcon[plant.primary_fuel] || '⚡';
    const cap = plant.capacity_mw != null ? (plant.capacity_mw >= 1000 ? (plant.capacity_mw / 1000).toFixed(1) + ' GW' : Math.round(plant.capacity_mw) + ' MW') : '—';
  ...
  // Detailed plant card (expandable per plant)
  let detailRows = '';
  for (let i = 0; i < sorted.length; i++) {
    ...
    const sourceInfo = [plant.source, plant.geolocation_source, plant.generation_data_source].filter(Boolean).join(' · ');
  ...
  return `<div class="pp-section" id="pp-section-${qid}" style="display:none">
    <div class="pp-section-hdr">
      <span>⚡ Power Plants — ${escHtml(p.city_name || '')} (${p.count} total, ${p.total_capacity_mw >= 1000 ? (p.total_capacity_mw / 1000).toFixed(1) + ' GW' : Math.round(p.total_capacity_mw) + ' MW'})</span>
...
// Expose toggle for onclick handler
...
function renderInfobox(city, images, wpExtra, wpUrl, fromCache) {
  ...
  const sistersHtml = city.sister_cities && city.sister_cities.length
    ...
    + (city.sister_cities.length > 8 ? ` <span style="color:var(--text-faint)">+${city.sister_cities.length - 8} more</span>` : '')
    : null;
  ...
  const gdpHtml = wpExtra && wpExtra.gdp
    ? `${wpExtra.gdp} <span style="color:var(--text-faint);font-size:0.75em">(local currency)</span>`
    : null;
  ...
  const infoChips = `<div class="info-chips">
    ...
    ${(() => { const g = S.gawcCities[city.qid]; if (!g) return ''; const col = GAWC_TIER_COLOR[g.tier] || '#8b949e'; return infoChip('World City', `<span class="gawc-tier-chip" style="background:${col}22;color:${col};border:1px solid ${col}55">${escHtml(g.tier)}</span>`, true, false, 'gawc_score'); })()}
    ${(() => { const a = S.airportData[city.qid]; if (!a) return ''; return infoChip('Airport', `<span style="color:var(--accent);font-weight:600">✈ ${fmtNum(a.directDestinations)}</span> <span style="color:var(--text-secondary);font-size:0.78em">direct routes · ${escHtml(a.iata)}</span>`, true, false, 'directDestinations'); })()}
    ${(() => { const p = S.portData[city.qid]; if (!p) return ''; return infoChip('Port', `<span style="color:var(--accent);font-weight:600">⚓ ${p.teu_millions}M TEU</span> <span style="color:var(--text-secondary);font-size:0.78em">${escHtml(p.port)}</span> <span style="color:var(--text-muted);font-size:0.72em">#${p.rank} globally</span>`, true, false, 'port_teu'); })()}
    ${(() => { const t = S.tourismData[city.qid]; if (!t) return ''; return infoChip('Tourism', `<span style="color:var(--gold);font-weight:600">${t.visitors_millions}M</span> <span style="color:var(--text-secondary);font-size:0.78em">visitors/yr</span> <span style="color:var(--text-muted);font-size:0.72em">#${t.rank} globally · ${t.year}</span>`, true, false, 'tourism_visitors'); })()}
    ${(() => { const pt = S.patentData[city.qid]; if (!pt) return ''; return infoChip('Patents', `<span style="color:var(--purple);font-weight:600">${fmtNum(pt.patents_annual)}</span> <span style="color:var(--text-secondary);font-size:0.78em">patents/yr</span> <span style="color:var(--text-faint);font-size:0.72em">${(pt.top_fields||[]).slice(0,2).join(', ')}</span>`, true, false, 'patents_annual'); })()}
    ${(() => { const cl = S.colData[city.qid]; if (!cl) return ''; return infoChip('Cost of Living', `<span style="color:var(--univ-gold);font-weight:600">${cl.col_index}</span> <span style="color:var(--text-secondary);font-size:0.78em">CoL index</span> <span style="color:var(--text-faint);font-size:0.72em">Rent:${cl.rent_index} Groc:${cl.grocery_index} Rest:${cl.restaurant_index}</span> <span style="color:var(--text-muted);font-size:0.72em">#${cl.col_rank} globally</span>`, true, false, 'col_index'); })()}
    ${(() => { const su = S.startupData[city.qid]; if (!su) return ''; return infoChip('Startups', `<span style="color:var(--purple);font-weight:600">🦄 ${su.unicorns}</span> <span style="color:var(--text-secondary);font-size:0.78em">unicorns</span> <span style="color:var(--success);font-size:0.78em">$${su.total_funding_bn}B</span> <span style="color:var(--text-secondary);font-size:0.78em">funding</span> <span style="color:var(--text-muted);font-size:0.72em">#${su.ecosystem_rank} globally</span>`, true, false, 'startup_unicorns'); })()}
    ${(() => {
      const aq = S.airQualityData[city.qid];
      if (!aq) return '';
      ...
      return infoChip('Air Quality', `<span style="color:${col};font-weight:700">${aq.pm25}</span><span style="color:var(--text-secondary);font-size:0.78em"> µg/m³ PM2.5</span> ${badge} <span style="color:var(--text-muted);font-size:0.72em">${aq.year}</span>`, true, false, 'who_pm25');
    })()}
    ${(() => { const m = S.metroTransitData[city.qid]; if (!m) return ''; const mr = S.metroData[city.qid]; const parts = []; if (mr && mr.ridership_millions) parts.push(`<span style="color:var(--accent);font-weight:600">${fmtNum(mr.ridership_millions)}M</span><span style="color:var(--text-secondary);font-size:0.78em"> riders/yr</span>`); if (m.lines) parts.push(`<span style="color:var(--text-primary);font-weight:600">${m.lines}</span><span style="color:var(--text-secondary);font-size:0.78em"> lines</span>`); if (m.stations) parts.push(`<span style="color:var(--text-primary);font-weight:600">${m.stations}</span><span style="color:var(--text-secondary);font-size:0.78em"> stations</span>`); if (m.opened) parts.push(`<span style="color:var(--text-muted);font-size:0.72em">since ${m.opened}</span>`); return infoChip('Metro', `🚇 ${parts.join(' · ')} <span style="color:var(--text-secondary);font-size:0.73em">${escHtml(m.name)}</span>`, true, false, 'metroStations'); })()}
    ${(() => { const n = S.nobelCitiesData[city.qid]; if (!n) return ''; const cats = Object.entries(n.byPrize||{}).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<span title="${escHtml(k)}" style="font-size:0.8em">${v}×${escHtml(k.slice(0,3))}</span>`).join(' '); return infoChip('Nobels', `<span style="color:var(--gold);font-weight:700">${n.total}</span> <span style="color:var(--text-secondary);font-size:0.78em">laureates</span> <span style="color:var(--text-faint);font-size:0.76em">(${cats})</span>`, true, false, 'nobelLaureates'); })()}
    ${(() => {
      const p = S.powerData[city.qid];
      if (!p) return '';
      const fuelIcon = { 'Hydro':'💧', 'Nuclear':'☢️', 'Coal':'⚫', 'Gas':'🔥', 'Oil':'🛢️', 'Solar':'☀️', 'Wind':'🌬️', 'Biomass':'🌿', 'Geothermal':'🌋', 'Waste':'🗑️' };
      const icon = fuelIcon[p.primary_fuel] || '⚡';
      const capStr = p.total_capacity_mw >= 1000 ? `${(p.total_capacity_mw/1000).toFixed(1)} GW` : `${Math.round(p.total_capacity_mw)} MW`;
      const fuelBreakdown = p.fuel_breakdown ? Object.entries(p.fuel_breakdown).sort((a,b)=>b[1]-a[1]).map(([f,cap])=>{
        const fi = fuelIcon[f] || '⚡';
        const capS = cap >= 1000 ? (cap/1000).toFixed(1) + ' GW' : Math.round(cap) + ' MW';
        return `<span style="font-size:0.7em;white-space:nowrap" title="${escHtml(f)}: ${capS}">${fi}</span>`;
      }).join(' ') : '';
      const attr = p.plants && p.plants.length > 0
        ? safeOnclick('togglePowerPlantDetail', city.qid) + ' title="Click to see all ' + p.count + ' power plants" style="cursor:pointer"'
        : '';
    ...
    ${(() => {
      ...
      out += infoChip('Hot/Cold',   `<span style="color:${hotCl};font-weight:600">${ca.hottestTemp.toFixed(1)}°C</span> <span style="color:var(--text-faint)">/</span> <span style="color:${coldCl};font-weight:600">${ca.coldestTemp.toFixed(1)}°C</span>`, true, false, 'hottestMonthTemp');
...
`;
  ...
  const wbSec = wb ? `<tr><td colspan="2" class="wiki-info-section-head">Country · ${escHtml(wb.name || city.country || city.iso)} <span style="color:var(--text-muted);font-size:0.75em">(World Bank)</span></td></tr>
  ...
  const climateHtml = (() => {
    ...
    const sourceLabel = cl.location
      ? `<div style="font-size:0.63rem;color:var(--text-muted);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(cl.location)}</div>`
      : '';

    return `
      <div class="wiki-climate-wrap">
        <div class="wiki-climate-head">Climate <span style="font-size:0.65rem;color:var(--text-muted);font-weight:400">· hover for details</span></div>
  ...
  const univHtml = univList.length ? (function() {
    const rows = univList.slice(0, 20).map(u => {
      ...
      const rankBadge = rk ? `<span class="univ-rank">${rk.qs_rank <= 50 ? '🏆' : ''} QS#${rk.qs_rank} THE#${rk.the_rank}</span>` : '';
...
// (censusCities moved to state.js)
// (censusBusiness moved to state.js)
// (beaTradeData moved to state.js)
// (eurostatCities moved to state.js)
// (gawcCities moved to state.js)
// (japanPrefData moved to state.js)
// (usStatesData moved to state.js)
// (eurostatRegions moved to state.js)
// (canadaProvinces moved to state.js)
// (australiaStates moved to state.js)
// (airQualityData moved to state.js)
// (universitiesData moved to state.js)
// (fbiCrimeData moved to state.js)
// (eciData moved to state.js)
// (metroTransitData moved to state.js)
// (nobelCitiesData moved to state.js)

// ── Filter panel state ────────────────────────────────────────────────────────
// (_filterAvail moved to state.js)
// (_filterValue moved to state.js)
// ── Dot display state (unified for filter + heatmap) ─────────────────────────
// (_matchedVisMode moved to state.js)
// (_matchedColorMode moved to state.js)
// (_otherShowMode moved to state.js)
// (_otherColorMode moved to state.js)

// ── Heatmap state ─────────────────────────────────────────────────────────────
// (_heatmapMetric moved to state.js)
// (_heatmapLayer moved to state.js)
// (_heatPalette moved to state.js)
// (_heatNormP95 moved to state.js)
// User-tunable heatmap parameters (exposed via sliders in the filter panel)
// (_heatRadius moved to state.js)
// (_heatBlur moved to state.js)
// (_heatMinOpacity moved to state.js)
// (airportData moved to state.js)
// (zillowData moved to state.js)
// (climateExtra moved to state.js)
// (portData moved to state.js)
// (tourismData moved to state.js)
// (metroData moved to state.js)
// (patentData moved to state.js)
// (subnatHdiData moved to state.js)
// (unescoSites moved to state.js)
// (unescoLayer moved to state.js)
// (unescoOn moved to state.js)
// (cableData moved to state.js)
// (cableLayer moved to state.js)
// (cableOn moved to state.js)
// (airRouteData moved to state.js)
// (airRouteLayer moved to state.js)
// (airRouteOn moved to state.js)
// (colData moved to state.js)
// (uniRankings moved to state.js)
// (startupData moved to state.js)
// (ecbData moved to state.js)
// (ecbBonds moved to state.js)
// (bojData moved to state.js)
// (oecdData moved to state.js)
// (comtradeData moved to state.js)
// (noaaClimate moved to state.js)
// (_cpCurrentIso2 moved to state.js)
// (_cpEscListener moved to state.js)
// (_cpHighlightedLayer moved to state.js)
function _cpMapClickHandler() { if (S._cpCurrentIso2) closeCountryPanel(); }
...
// (censusColorMetric moved to state.js)
// (cityAqMode moved to state.js)
...
async function toggleAqMode() {
  ...
  if (S.cityAqMode) {
    ...
    if (S.censusColorMetric) { setCensusColorMetric(''); const cs = document.getElementById('census-metric-select'); if (cs) cs.value = ''; }
  } else {
  ...
  if (legendTitle) {
    legendTitle.textContent = S.cityAqMode
      ? 'Cities colored by WHO PM2.5 annual mean · click dot for details'
      : S.allCities.length.toLocaleString() + ' cities on S.map · circle size and color = population';
  }
}
...
function setCensusColorMetric(val) {
  ...
  if (S.censusColorMetric) {
    ...
    if (!S._filterAvail.census) {
      ...
      if (censusBtn) { censusBtn.classList.add('on'); censusBtn.textContent = 'ON'; }
      S._otherShowMode = 'hide';
      document.querySelectorAll('[data-other-vis]').forEach(b =>
        b.classList.toggle('active', b.dataset.otherVis === 'hide'));
      S._matchedColorMode = 'metric';
    ...
    if (S._filterAvail.census) {
      ...
      const censusBtn = document.querySelector('.filter-avail-btn[data-key="census"]');
      if (censusBtn) { censusBtn.classList.remove('on'); censusBtn.textContent = 'off'; }
      _updateFilterBadge();
    }
  }
...
const CITY_STAT_DEFS = {
  ...
  gawc_score: { label:'GaWC World City Rank', key: c=>S.gawcCities[c.qid]?.score ?? null,
                fmt: v => { const tier = Object.entries(GAWC_TIER_SCORE).find(([,s])=>s===v)?.[0]||''; return tier; },
                higherBetter:true },
  directDestinations: { label:'Direct Air Destinations', key: c=>S.airportData[c.qid]?.directDestinations ?? null,
                fmt: v => fmtNum(v) + ' airports', higherBetter:true },
  who_pm25:     { label:'PM2.5 Air Quality (WHO)',   key: c=>S.airQualityData[c.qid]?.pm25 ?? null,
                  fmt: v => v.toFixed(1) + ' μg/m³', higherBetter:false },
  fbi_violentPer100k:  { label:'Violent Crime Rate (FBI)',  key: c=>S.fbiCrimeData[c.qid]?.violentPer100k ?? null,
                          fmt: v => v.toFixed(0) + '/100k', higherBetter:false },
  fbi_propertyPer100k: { label:'Property Crime Rate (FBI)', key: c=>S.fbiCrimeData[c.qid]?.propertyPer100k ?? null,
                          fmt: v => v.toFixed(0) + '/100k', higherBetter:false },
  zhvi: { label:'Home Value (Zillow ZHVI)',  key: c=>S.zillowData[c.qid]?.zhvi  ?? null,
          fmt: v => '$' + fmtNum(Math.round(v)), higherBetter:null },
  zori: { label:'Rent Index (Zillow ZORI)',  key: c=>S.zillowData[c.qid]?.zori  ?? null,
  ...
  coldestMonthTemp: { label:'Coldest Month Avg °C', key: c => { const a = climateAnnual(getCityClimate(c)); return a ? a.coldestTemp : null; },
                      fmt: v => v.toFixed(1) + '°C', higherBetter:null },
  metroStations:    { label:'Metro Stations',     key: c => S.metroTransitData[c.qid]?.stations ?? null,
                      fmt: v => fmtNum(v) + ' stations', higherBetter:true },
  nobelLaureates:   { label:'Nobel Laureates',    key: c => S.nobelCitiesData[c.qid]?.total ?? null,
                      fmt: v => String(v), higherBetter:true },
  port_teu:         { label:'Port TEU (millions)', key: c => S.portData[c.qid]?.teu_millions ?? null,
                      fmt: v => v + 'M TEU', higherBetter:true },
  tourism_visitors: { label:'Tourism Visitors (millions)', key: c => S.tourismData[c.qid]?.visitors_millions ?? null,
                      fmt: v => v + 'M visitors', higherBetter:true },
  patents_annual:   { label:'Patents per Year',   key: c => S.patentData[c.qid]?.patents_annual ?? null,
                      fmt: v => fmtNum(v) + ' patents', higherBetter:true },
  metro_ridership:  { label:'Metro Ridership (millions)', key: c => S.metroData[c.qid]?.ridership_millions ?? null,
                      fmt: v => fmtNum(v) + 'M riders/yr', higherBetter:true },
  col_index:        { label:'Cost of Living Index',    key: c => S.colData[c.qid]?.col_index ?? null,
                      fmt: v => v + ' (NYC=100)', higherBetter:null },
  startup_unicorns: { label:'Unicorn Startups',       key: c => S.startupData[c.qid]?.unicorns ?? null,
                      fmt: v => v + ' unicorns', higherBetter:true },
  startup_funding:  { label:'VC Funding ($B)',         key: c => S.startupData[c.qid]?.total_funding_bn ?? null,
                      fmt: v => '$' + v + 'B', higherBetter:true },
};
...
function _statSourceAttr(metric) {
  var sources = {
    ...
    oecd_house_price_income: 'OECD · Analytical House Prices Indicators (HOUSE_PRICES)',
    oecd_broadband_per100: 'OECD · Broadband Statistics (BROADBAND_DB)',
    oecd_employment_rate: 'OECD · Labour Force Statistics (LFS)',
    oecd_life_satisfaction: 'OECD · Better Life Index (BLI)',
    oecd_gini: 'OECD · Income Distribution Database (IDD)',
    oecd_pension_spend: 'OECD · Social Expenditure Database (SOCX_AGG)',
    ...
    wb_who_uhc_index: 'WHO · Global Health Observatory (UHC_INDEX_REPORTED)',
    wb_who_hale: 'WHO · Global Health Observatory (WHOSIS_000002)',
    wb_who_health_spend_pc: 'WHO · Global Health Observatory (GHED_CHEGDP_SHA2011)',
    wb_who_oop_spend: 'WHO · Global Health Observatory (GHED_OOPSCHE_SHA2011)',
    wb_who_tb_incidence: 'WHO · Global Health Observatory (MDG_0000000020)',
    wb_who_hiv_prevalence: 'WHO · Global Health Observatory (MDG_0000000029)',
    wb_who_malaria_incidence: 'WHO · Global Health Observatory (MALARIA_EST_INCIDENCE)',
    wb_who_hepb_prevalence: 'WHO · Global Health Observatory (WHS4_117)',
    wb_who_obesity: 'WHO · Global Health Observatory (NCD_BMI_30A)',
    wb_who_tobacco: 'WHO · Global Health Observatory (M_Est_tob_curr_std)',
    wb_who_alcohol: 'WHO · Global Health Observatory (SA_0000001688)',
    wb_who_physical_inactivity: 'WHO · Global Health Observatory (NCD_PAC)',
    ...
    // Peace & Security
    wb_military_spend_gdp: 'World Bank · WDI (MS.MIL.XPND.GD.ZS)',
    wb_gpi_score: 'Global Peace Index 2024 · Institute for Economics & Peace',
    ...
    wb_pop_growth: 'World Bank · WDI (SP.POP.GROW)',
    wb_net_migration: 'World Bank · WDI (SM.POP.NETM)',
    wb_migrant_stock: 'World Bank · WDI (SM.POP.TOTL)',
    wb_female_labor: 'World Bank · WDI (SL.TLF.CACT.FE.ZS)',
    wb_safe_water: 'World Bank · WDI (SH.H2O.SMDW.ZS)',
    wb_research_articles: 'World Bank · WDI (IP.JRN.ARTC.SC)',
    // Nuclear Energy
...
const WB_STAT_DEFS = {
  ...
  wb_credit_sp:      { label:"S&P Credit Rating",       key:'credit_sp_num',     fmt: function(v){ return S._numToCredit.sp[v]     || v.toFixed(0); }, higherBetter:true },
  wb_credit_moodys:  { label:"Moody's Credit Rating",   key:'credit_moodys_num', fmt: function(v){ return S._numToCredit.moodys[v] || v.toFixed(0); }, higherBetter:true },
  wb_credit_fitch:   { label:"Fitch Credit Rating",     key:'credit_fitch_num',  fmt: function(v){ return S._numToCredit.fitch[v]  || v.toFixed(0); }, higherBetter:true },
...
async function openWikiSidebar(qid, cityName) {
  ...
  // Skeleton loading placeholders (no user data — safe static HTML)
  if (_infoEl) _infoEl.innerHTML = '<div class="skeleton-block">'
...
// ── Boot ──────────────────────────────────────────────────────────────────────
// ── Basemap & theme ──────────────────────────────────────────────────────────
// Placeholder tile for failed terrain loads (light gray with subtle grid pattern)
const _errorTileDataUrl = 'data:image/svg+xml,' + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256">' +
  '<rect fill="#e8e8e8" width="256" height="256"/>' +
  '<path d="M0 128h256M128 0v256" stroke="#d0d0d0" stroke-width="1"/>' +
  '</svg>'
);

// Track if we've already fallen back to ESRI terrain
let _terrainFallbackActive = false;
...
function _swapTileLayer() {
  ...
  // Build tile layer options with error handling
  const opts = {
  ...
  // Use ESRI fallback if OpenTopoMap failed previously
  let tileKey = key;
  ...
  // Add error placeholder for terrain (prone to outages)
  if (key === 'terrain') {
  ...
  // Handle tile errors - fall back to ESRI terrain on repeated failures
  if (key === 'terrain') {
    let errorCount = 0;
    S._tileLayer.on('tileerror', (err) => {
      errorCount++;
      // Use placeholder immediately for failed tiles
      if (err.tile) {
...
function _restoreFromHash() {
  ...
  if (params.choro) {
    ...
    // Defer toggling until choropleth data is loaded (handled after worldGeo loads)
    S._hashRestoreChoro = true;
  }
  // Restore city sidebar (defer until cities loaded)
  if (params.city) S._hashRestoreCity = params.city;
  // Restore country panel (defer until country data loaded)
  if (params.country) S._hashRestoreCountry = params.country;
}

// ── Keyboard navigation: extracted to src/keyboard-nav.js ────────────────────
...
async function init() {
  ...
  //   choroplethPane        350  — country fill polygons
  //   admin1Pane            370  — admin-1 boundary lines
  //   tradePane             390  — BEA trade-flow arc lines
  //   cityPane              400  — city dot markers
  //   cryptoPane            410  — crypto dot markers
  //   econPane              420  — economic center dots
  //   overlayLayersPane     430  — ports, satellites, etc.; above cities so overlay clicks win
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  // Restore saved basemap and theme
  ...
  // Restore cities visibility preference
  const savedCitiesMode = localStorage.getItem('wdm_citiesMode') || 'show';
  if (savedCitiesMode !== 'show') setCityDotMode(savedCitiesMode);

  // Initialize population range slider
  _initPopSlider();

  // Keyboard navigation + URL state
  initKeyboardNav({
    closeCountryCompare,
    closeComparePanel,
    closeWikiSidebar,
    closeCountryPanel: () => { document.getElementById('country-panel').classList.remove('open'); S._cpCurrentIso2 = null; },
    closeCorpPanel,
    closeStatsPanel,
    closeTradePanelFn,
    toggleFilterPanel,
  ...
  // Click outside closes the more-layers dropdown
  ...
  try {
    // Lazy-loaded on demand (via createLazyLoader above):
    ...
    //   companies-index.json 2.7 MB — on first company lookup
    //   companies-detail.json  41 MB — on first company detail view

    // ── Critical: fetch only the two datasets needed for initial render ──
    ...
    function _matchCityQid(name, isoOrCountry) {
    ...
    function _loadCityArrayFromUrl(url, label, assign) {
      // Check kdb first
      var stem = url.replace(/^\//, '').replace(/\.json(\?.*)?$/, '');
      var kdbData = _kdbGet(stem);
      if (kdbData !== null) {
    ...
    // These are not needed for the initial map render and load non-blocking.
    ...
    // Mobile: close topbar on map tap
  ...
  if (mbtn) mbtn.classList.remove('open');
    ...
    // Restore URL hash state (deferred until data is ready)
    ...
    if (!_fxLoadFromLS()) {
      // First visit — set date to today and fetch latest rates
...
// ── Time slider state ──
// (_choroYear moved to state.js)
// (_choroPlayTimer moved to state.js)
...
function buildChoropleth() {
  ...
  if (S.choroplethLayer) { S.map.removeLayer(S.choroplethLayer); S.choroplethLayer = null; }
  S._choroLayerByIso = {};
    ...
    onEachFeature: function (feature, layer) {
      ...
      layer.on({
        mouseover: function (e) {
          // Reset previous hover if any (prevents stuck highlights on fast mouse movement)
          if (S._choroHoverTarget && S._choroHoverTarget !== e.target) {
            try { S.choroplethLayer.resetStyle(S._choroHoverTarget); } catch(ex) {}
          }
  ...
  if (S.wikiLayer) { S.map.removeLayer(S.wikiLayer); S.wikiLayer.addTo(S.map); }

  // Update legend
  updateChoroLegend(ind, range, covered);
}

// Fast restyle for time slider — recolors existing polygons without destroying/recreating the layer
function _restyleChoropleth() {
  if (!S.choroplethLayer || !S.worldGeo || !S.choroOn) { buildChoropleth(); return; }
  var ind = CHORO_INDICATORS.find(function(i) { return i.key === S.activeChoroKey; }) || CHORO_INDICATORS[0];
...
// ── Time slider for choropleth ────────────────────────────────────────
function _updateChoroSliderVisibility() {
  var ind = CHORO_INDICATORS.find(function(i) { return i.key === S.activeChoroKey; });
  ...
  if (ind && ind.histKey) {
    ...
    // Determine year range from history data
...
function _stopChoroPlay() {
  if (S._choroPlayTimer) { clearInterval(S._choroPlayTimer); S._choroPlayTimer = null; }
  var btn = document.getElementById('choro-play-btn');
  if (btn) btn.textContent = '▶';
}

function toggleChoroPlay() {
  if (S._choroPlayTimer) { _stopChoroPlay(); return; }
  var rangeEl = document.getElementById('choro-year-range');
  ...
  S._choroPlayTimer = setInterval(function() {
    cur++;
    if (cur > maxY) { _stopChoroPlay(); S._choroYear = null; rangeEl.value = maxY; document.getElementById('choro-year-label').textContent = 'Year: Latest'; _restyleChoropleth(); return; }
    rangeEl.value = cur;
    S._choroYear = cur;
    document.getElementById('choro-year-label').textContent = 'Year: ' + cur;
    _restyleChoropleth();
...
// Wire up slider input (throttled to avoid restyle on every pixel of drag)
(function() {
  var rangeEl = document.getElementById('choro-year-range');
  if (!rangeEl) return;
  var pending = null;
  rangeEl.addEventListener('input', function() {
    if (pending) return;
    pending = requestAnimationFrame(function() { pending = null; _onChoroYearChange(); });
  });
})();
...
function openCountryPanel(iso2) {
  ...
  // highlight selected country border on choropleth (O(1) lookup instead of iterating all layers)
  if (S.choroplethLayer) {
    if (S._cpHighlightedLayer) {
      try { S.choroplethLayer.resetStyle(S._cpHighlightedLayer); } catch(ex) {}
    }
  ...
  S._cpEscListener = function(e) { if (e.key === "Escape") closeCountryPanel(); };
...
// (_cpWorldMaxCache moved to state.js)
...
// (_cpOecdMaxCache moved to state.js)
...
// ── Clear region selection and re-render country panel ───────────────────────
...
function _renderCountryPanel(iso2) {
  ...
  var gaugeHtml =
    ...
    (function() {
      ...
      var hazardRows = '';
      if (Number.isFinite(r.earthquake)) hazardRows += '<span title="Earthquake risk">' + (r.earthquake >= 5 ? '🔴' : r.earthquake >= 2 ? '🟡' : '⚪') + ' Quake ' + r.earthquake.toFixed(1) + '</span> ';
      if (Number.isFinite(r.flood)) hazardRows += '<span title="Flood risk">' + (r.flood >= 5 ? '🔴' : r.flood >= 2 ? '🟡' : '⚪') + ' Flood ' + r.flood.toFixed(1) + '</span> ';
      if (Number.isFinite(r.cyclone)) hazardRows += '<span title="Cyclone risk">' + (r.cyclone >= 5 ? '🔴' : r.cyclone >= 2 ? '🟡' : '⚪') + ' Cyclone ' + r.cyclone.toFixed(1) + '</span> ';
      if (Number.isFinite(r.drought)) hazardRows += '<span title="Drought risk">' + (r.drought >= 5 ? '🔴' : r.drought >= 2 ? '🟡' : '⚪') + ' Drought ' + r.drought.toFixed(1) + '</span> ';
      if (Number.isFinite(r.conflict) && r.conflict > 0) hazardRows += '<span title="Conflict risk">' + (r.conflict >= 5 ? '🔴' : r.conflict >= 2 ? '🟡' : '⚪') + ' Conflict ' + r.conflict.toFixed(1) + '</span> ';
    ...
    (function() {
      ...
      if (!c) return '';
      var rows = '';
      var maxRefOrigin = _cpWorldMax('refugees_by_origin') || 15000000;
      var maxRefAsylum = _cpWorldMax('refugees_by_asylum') || 4000000;
      var maxForestLoss = _cpWorldMax('forest_loss_sqkm') || 50000;
      var maxForestCover = _cpWorldMax('forest_cover_2000_sqkm') || 9000000;
      ...
      if (Number.isFinite(c.forest_loss_sqkm)) {
        rows += _cpGaugeRow('Forest loss', c.forest_loss_sqkm, maxForestLoss, ' km²', 'cp-red', 'gfw_forest_loss', iso2);
        if (c.forest_loss_year) rows += '<span class="cp-gauge-year">(' + c.forest_loss_year + ')</span>';
      }
      if (Number.isFinite(c.forest_cover_2000_sqkm)) {
        rows += _cpGaugeRow('Forest cover 2000', c.forest_cover_2000_sqkm, maxForestCover, ' km²', 'cp-green', 'gfw_forest_cover', iso2);
      }
    ...
    (Number.isFinite(cd.energy_coal_pct)
      ? '<div class="cp-gauge-section-hdr">Energy Mix</div>' +
        ...
        (Number.isFinite(cd.nuclear_reactors)    ? _cpGaugeRow('Reactors',        cd.nuclear_reactors,       _cpWorldMax('nuclear_reactors'),       '',    'cp-amber', 'wb_nuclear_reactors',  iso2) : '') +
        (Number.isFinite(cd.nuclear_capacity_gw) ? _cpGaugeRow('Nuclear capacity', cd.nuclear_capacity_gw,   _cpWorldMax('nuclear_capacity_gw'),   ' GW', 'cp-amber', 'wb_nuclear_capacity',  iso2) : '') +
        (Number.isFinite(cd.nuclear_generation_twh) ? _cpGaugeRow('Nuclear output', cd.nuclear_generation_twh, _cpWorldMax('nuclear_generation_twh'), ' TWh','cp-amber','wb_nuclear_generation', iso2) : '')
      : '') +
    // ── Research Output (OpenAlex) ───────────────────────────────────────
    (function() {
      var c = S.countryData[iso2];
      ...
      var rows = '';
      var maxPapers = _cpWorldMax('research_papers') || 50000000;
      var maxCites  = _cpWorldMax('research_citations') || 1000000000;
    ...
    (function() {
      ...
      var riskColor = c.inform_risk >= 6.5 ? '#f85149' : c.inform_risk >= 5.0 ? '#d29922' : c.inform_risk >= 3.5 ? '#e3b341' : '#3fb950';
      var rows = '';
...
function _buildCountryDataCaches() {
  ...
  regions.forEach(function(region) {
    ...
    rankKeys.forEach(function(key) {
      var peers = [];
      for (var iso in S.countryData) {
        if (!S.countryData[iso] || S.countryData[iso].region !== region) continue;
        var v = S.countryData[iso][key];
...
function clearTradeArrows() {
  if (S.tradeArrowLayer) { S.map.removeLayer(S.tradeArrowLayer); S.tradeArrowLayer = null; }
}
...
// (_choroControlsInited moved to state.js)
async function toggleChoropleth() {
  ...
  if (S.choroOn) {
    // Turn off admin-1 regions if on (mutually exclusive)
    if (S.admin1On) { await toggleAdmin1Global(); }
    const origText = btn.textContent;
    btn.textContent = 'Loading…';
    btn.disabled = true;
    await _worldGeoLoader.ensure();
    if (!S._choroControlsInited) { initChoroControls(); S._choroControlsInited = true; }
...
let _nationsPanelOpen = false;
export function toggleNationsPanel() {
...
function _buildNationsList() {
  ...
  body.innerHTML = '';
  // Keep hidden select for choro-select compatibility
...
// (subnatData moved to state.js)
// (subnatFiles moved to state.js)
...
function _admin1RegionVal(iso2, regionName) {
  ...
  if (sd) {
    ...
    if (!result) {
      for (var k in sd) {
        if (sd[k] && sd[k].name === regionName) { result = sd[k]; break; }
      }
    }
  }
  ...
  // Overlay subnational HDI (enriches existing regionData or creates new)
  var hdiCountry = S.subnatHdiData[iso2];
  if (hdiCountry) {
    // Handle two-level structure (GB, FR): { regions: {...}, la_to_region/dept_to_region: {...} }
    ...
    // Also try via subnatData mappings for consistency
...
// Ensure topojson-client is loaded (one-time)
...
// Build an L.geoJSON layer for one country's admin-1 boundaries
  ...
  return L.geoJSON(geoData, {
    ...
    onEachFeature: function(feature, layer) {
      ...
      layer.on({
        click: function(e) {
          L.DomEvent.stopPropagation(e);
          S._selectedRegion = { iso2: iso2, name: name, code: code, data: regionData };
          openCountryPanel(iso2);
        },
        mouseover: function(e) {
          // Reset previous hover target to prevent stuck highlights on fast scrolling
        ...
        mouseout: function(e) {
          ...
          if (S._admin1HoverTarget === e.target) { S._admin1HoverTarget = null; S._admin1HoverIso = null; }
        },
      });
    },
  });
...
// Load and display admin-1 for a single country (returns promise, deduped)
function _loadAdmin1Country(iso2) {
  if (S.admin1Layers[iso2]) return Promise.resolve();  // already on S.map
  if (S._admin1Loading[iso2]) return S._admin1Loading[iso2];  // in flight

  S._admin1Loading[iso2] = (async function() {
    try {
      // Load TopoJSON + subnational data in parallel
      var fetches = [];
      if (!admin1Cache[iso2]) {
        fetches.push(
      ...
      if (S.admin1On) {  // still on? (user might have toggled off while loading)
        layer.addTo(S.map);
...
// Pre-computed country bounding boxes (computed once from worldGeo)
// (_countryBounds moved to state.js)
function _ensureCountryBounds() {
  ...
  for (var i = 0; i < S.worldGeo.features.length; i++) {
    ...
    // Compute bounding box from raw coordinates (no L.geoJSON overhead)
    ...
    else if (g.type === 'MultiPolygon') g.coordinates.forEach(function(p) { p.forEach(processRing); });
    if (found) {
...
// Get ISO2 codes of countries visible in the current map viewport
...
// Load admin-1 for all visible countries (called on toggle + map move)
// (_admin1UpdateTimer moved to state.js)
function _updateAdmin1Visible() {
  if (!S.admin1On) return;
  var visible = _visibleCountries();
  var toLoad = visible.filter(function(iso2) { return !S.admin1Layers[iso2] && !S._admin1Loading[iso2]; });
  if (toLoad.length === 0) return;
  // Prioritize smaller files first for fast visual feedback
  toLoad.sort(function(a, b) { return (S.admin1Index[a].kb || 0) - (S.admin1Index[b].kb || 0); });
  // Load up to 10 at a time, then chain next batch when done
  var batch = toLoad.slice(0, 10);
  var promises = batch.map(function(iso2) { return _loadAdmin1Country(iso2); });
  Promise.all(promises).then(function() {
    // After batch completes, load next batch if any remain
...
// Global toggle: show/hide all admin-1 boundaries
async function toggleAdmin1Global() {
  S.admin1On = !S.admin1On;
  var btn = document.getElementById('admin1-toggle-btn');

  if (S.admin1On) {
    // Turn off national borders if on (mutually exclusive)
    if (S.choroOn) { await toggleChoropleth(); }
    btn.textContent = 'Loading…';
    btn.disabled = true;
    ...
    btn.disabled = false;
    btn.classList.add('on');
    // Start loading visible countries
...
// (_selectedRegion moved to state.js)

// Build HTML for the selected region section in the country panel
function _buildRegionSection() {
  var r = S._selectedRegion;
  if (!r || !r.data) { S._selectedRegion = null; return ''; }
  var rd = r.data;
  ...
  // Economy
  if (rd.gdp_bn_eur != null) { html += _cpInfoRow('GDP', '\u20ac' + fmtNum(rd.gdp_bn_eur) + 'B'); if (rd.gdp_per_capita_eur != null) html += _cpInfoRow('GDP/cap', '\u20ac' + fmtNum(rd.gdp_per_capita_eur)); }
  if (rd.gva_bn_gbp != null) { html += _cpInfoRow('GVA', '\u00a3' + fmtNum(rd.gva_bn_gbp) + 'B'); if (rd.gva_per_capita_gbp != null) html += _cpInfoRow('GVA/cap', '\u00a3' + fmtNum(rd.gva_per_capita_gbp)); }
  ...
  if (rd.gdp_bn_cny != null) { html += _cpInfoRow('GDP', '\u00a5' + fmtNum(rd.gdp_bn_cny) + 'B'); if (rd.gdp_per_capita_cny != null) html += _cpInfoRow('GDP/cap', '\u00a5' + fmtNum(rd.gdp_per_capita_cny)); }
  ...
  if (rd.perCapitaIncomeJpy != null) html += _cpInfoRow('Per-cap income', '\u00a5' + fmtNum(Math.round(rd.perCapitaIncomeJpy)));
  ...
  S._selectedRegion = null; // consumed
  return html;
}

function _cpInfoRow(label, value) {
  return '<div class="cp-gauge-row"><span class="cp-gauge-lbl">' + escHtml(label) +
    '</span><span class="cp-gauge-info">' + value + '</span></div>';
}

function openRegionPanel(iso2, regionName, code, regionData) {
  ...
  // Country info
  var cd = S.countryData[iso2] || {};
  var countryName = cd.name || iso2;

  // Header
  header.innerHTML =
    '<span class="cp-flag">' + isoToFlag(iso2) + '</span>' +
    '<div style="flex:1;min-width:0">' +
      '<div class="rp-region-name">' + escHtml(regionName) + '</div>' +
      '<div class="rp-region-meta">' + escHtml(countryName) +
        (code ? ' · ' + escHtml(code) : '') + '</div>' +
    '</div>' +
    '<button class="rp-close" onclick="closeRegionPanel()">\u00d7</button>';
  ...
  if (regionData) {
    ...
    html += '<div class="rp-section-title">Economy</div>';

    // GDP / GVA variants
    if (regionData.gdp_bn_eur != null) {
      html += _rpRow('GDP', '€' + fmtNum(regionData.gdp_bn_eur) + 'B');
      if (regionData.gdp_per_capita_eur != null)
        html += _rpRow('GDP per capita', '€' + fmtNum(regionData.gdp_per_capita_eur));
      if (regionData.gdp_year) html += _rpRow('Year', regionData.gdp_year);
    }
    if (regionData.gva_bn_gbp != null) {
      html += _rpRow('GVA', '£' + fmtNum(regionData.gva_bn_gbp) + 'B');
      if (regionData.gva_per_capita_gbp != null)
        html += _rpRow('GVA per capita', '£' + fmtNum(regionData.gva_per_capita_gbp));
      if (regionData.gva_year) html += _rpRow('Year', regionData.gva_year);
    }
    if (regionData.gdp_pps_eu100 != null)
      html += _rpRow('GDP (PPS, EU=100)', regionData.gdp_pps_eu100);
    if (regionData.pcpi != null)
      html += _rpRow('Per-capita income', '$' + fmtNum(Math.round(regionData.pcpi)));
    if (regionData.gdp_bn_cny != null) {
      html += _rpRow('GDP', '¥' + fmtNum(regionData.gdp_bn_cny) + 'B');
      if (regionData.gdp_per_capita_cny != null)
        html += _rpRow('GDP per capita', '¥' + fmtNum(regionData.gdp_per_capita_cny));
    }
    if (regionData.grdp_bn_krw != null) {
      html += _rpRow('GRDP', '₩' + fmtNum(Math.round(regionData.grdp_bn_krw)) + 'B');
      if (regionData.grdp_per_capita_krw != null)
        html += _rpRow('GRDP per capita', '₩' + fmtNum(regionData.grdp_per_capita_krw));
    }
    if (regionData.gsdp_bn_inr != null) {
      html += _rpRow('GSDP', '₹' + fmtNum(Math.round(regionData.gsdp_bn_inr)) + 'B');
      if (regionData.gsdp_per_capita_inr != null)
        html += _rpRow('GSDP per capita', '₹' + fmtNum(regionData.gsdp_per_capita_inr));
    }
    if (regionData.gsp_bn_aud != null)
      html += _rpRow('GSP', 'A$' + fmtNum(regionData.gsp_bn_aud) + 'B');
    if (regionData.gdp_bn_cad != null)
      html += _rpRow('GDP', 'C$' + fmtNum(regionData.gdp_bn_cad) + 'B');
    if (regionData.perCapitaIncomeJpy != null)
      html += _rpRow('Per-capita income', '¥' + fmtNum(Math.round(regionData.perCapitaIncomeJpy)));

    // Labour section
    if (regionData.unemployment_rate != null || regionData.unemployment_year != null) {
      html += '<div class="rp-section-title">Labour</div>';
      if (regionData.unemployment_rate != null)
        html += _rpRow('Unemployment rate', regionData.unemployment_rate + '%');
      if (regionData.unemployment_year != null)
        html += _rpRow('Year', regionData.unemployment_year);
    }

    // Human Development section
    if (regionData.hdi != null) {
      html += '<div class="rp-section-title">Human Development</div>';
      html += _rpRow('HDI', regionData.hdi.toFixed(3));
      if (regionData.hdi_health != null) html += _rpRow('Health index', regionData.hdi_health.toFixed(3));
      if (regionData.hdi_education != null) html += _rpRow('Education index', regionData.hdi_education.toFixed(3));
      if (regionData.hdi_income != null) html += _rpRow('Income index', regionData.hdi_income.toFixed(3));
      if (regionData.hdi_year != null) html += _rpRow('Year', regionData.hdi_year);
    }

    // Demographics section
    if (regionData.population != null || regionData.population_m != null || regionData.urbanization_pct != null) {
      html += '<div class="rp-section-title">Demographics</div>';
      if (regionData.population_m != null)
        html += _rpRow('Population', fmtNum(regionData.population_m) + 'M');
      else if (regionData.population != null)
        html += _rpRow('Population', fmtNum(regionData.population));
      if (regionData.urbanization_pct != null)
        html += _rpRow('Urbanization', regionData.urbanization_pct + '%');
    }

    // Data source
    if (regionData.data_source)
      html += '<div class="rp-section-title">Source</div>' +
              '<div style="font-size:.72rem;color:var(--text-faint);padding:4px 0">' + escHtml(regionData.data_source) + '</div>';
  } else {
    html += '<div class="rp-no-data">No economic data available for this region.</div>';
    html += '<div style="font-size:.72rem;color:var(--text-muted);padding:4px 0">Boundary data: Natural Earth 10m admin-1</div>';
  }
  ...
  _mobileBackdropOn();
  if (S._rpEscListener) document.removeEventListener('keydown', S._rpEscListener);
  S._rpEscListener = function(e) { if (e.key === 'Escape') closeRegionPanel(); };
  document.addEventListener('keydown', S._rpEscListener);
}
...
function _rpRow(label, value) {
  return '<div class="rp-stat-row">' +
    '<span class="rp-stat-label">' + escHtml(label) + '</span>' +
    '<span class="rp-stat-val">' + value + '</span>' +
  '</div>';
}
...
// Extracted to src/unesco-layer.js
import { toggleUnescoLayer, buildUnescoLayer } from './unesco-layer.js';

// ── Submarine Cables layer ────────────────────────────────────────────────────
// Extracted to src/cable-layer.js
import { toggleCableLayer, buildCableLayer } from './cable-layer.js';

// ── Air Routes layer ──────────────────────────────────────────────────────────
// Extracted to src/air-route-layer.js
import { toggleAirRouteLayer, buildAirRouteLayer } from './air-route-layer.js';

// ── Tectonic plate boundaries ─────────────────────────────────────────────────
// Extracted to src/tectonic-plates-layer.js
import { toggleTectonicLayer, toggleMoreLayers, _setLayerBtn } from './tectonic-plates-layer.js';

// ── Space launch sites layer ─────────────────────────────────────────────────
// Extracted to src/launch-site-layer.js
import { toggleLaunchSiteLayer, _buildLaunchSiteLayer } from './launch-site-layer.js';

// ── Marine EEZ boundaries layer ──────────────────────────────────────────────
// Extracted to src/eez-layer.js
import { toggleEezLayer, _buildEezLayer } from './eez-layer.js';

// ── ISS live position tracker ────────────────────────────────────────────────
// Extracted to src/iss-layer.js
import { toggleIssTracker } from './iss-layer.js';

// ── OpenSky live aircraft tracker ─────────────────────────────────────────────
// Extracted to src/aircraft-layer.js
import { toggleAircraftLayer } from './aircraft-layer.js';

// Earthquake layer — extracted to src/earthquake-layer.js
import { toggleEarthquakeLayer, _buildEarthquakeLayer } from './earthquake-layer.js';

// Volcano layer — extracted to src/volcano-layer.js
import { toggleVolcanoLayer, _buildVolcanoLayer } from './volcano-layer.js';

// ── NASA FIRMS Wildfire Layer ──────────────────────────────────────────────────
// Extracted to src/firms-layer.js
import { toggleWildfireLayer } from './firms-layer.js';

// ── NASA EONET Natural Events Layer ────────────────────────────────────────────
// Extracted to src/eonet-layer.js
import { toggleEonetLayer, _buildEonetLayer } from './eonet-layer.js';

// ── Protected Areas Layer ─────────────────────────────────────────────────────
// Extracted to src/protected-areas-layer.js
import { toggleProtectedAreasLayer, _buildProtectedAreasLayer } from './protected-areas-layer.js';

// ── Shipping Ports Layer ──────────────────────────────────────────────────────
// Extracted to src/vessel-ports-layer.js
import { toggleVesselPortsLayer, _buildVesselPortsLayer } from './vessel-ports-layer.js';

// ── PeeringDB Internet Infrastructure Layer ────────────────────────────────────
// Extracted to src/peeringdb-layer.js
import { togglePeeringdbLayer, _buildPeeringdbLayer } from './peeringdb-layer.js';

// ── WAQI Air Quality Layer ─────────────────────────────────────────────────────
// Extracted to src/waqi-layer.js
import { toggleWaqiLayer, _buildWaqiLayer } from './waqi-layer.js';

// ── Open-Meteo Weather Layer ───────────────────────────────────────────────────
// Extracted to src/weather-layer.js
import { toggleWeatherLayer, _buildWeatherLayer } from './weather-layer.js';

// ── Celestrak Satellite Layer ──────────────────────────────────────────────────
// Extracted to src/satellite-layer.js
import { toggleSatelliteLayer, _buildSatelliteLayer } from './satellite-layer.js';

// ── UNESCO Intangible Heritage Layer ───────────────────────────────────────────
// Extracted to src/unesco-ich-layer.js
import { toggleUnescoIchLayer, _buildUnescoIchLayer } from './unesco-ich-layer.js';

// ── Phase 2 Layers: extracted to src/layers-phase2.js ───────────────────────

// ── Phase 3 Layers ────────────────────────────────────────────────────────────
// FlightAware Aircraft Tracking
// Extracted to src/flightaware-layer.js
import { toggleFlightAwareLayer, _buildFlightAwareLayer } from './flightaware-layer.js';

// MarineTraffic Vessel Tracking
// Extracted to src/marine-traffic-layer.js
import { toggleMarineTrafficLayer, _buildMarineTrafficLayer } from './marine-traffic-layer.js';

// ── Econ layer (corporate economy dots, clustering, expansion) ────────────────
// Extracted to src/econ-layer.js
import {
  buildEconLayer, toggleEconLayer, collapseEconCluster, expandEconCluster,
  econDotColor, _econCellDeg, _clearExpandedLayers, _convexHull, _arcLine
} from './econ-layer.js';
...
export function closeAllMobileSheets() {
  ...
  if (listPanel) { listPanel.classList.remove('panel-open'); }
  document.querySelectorAll('.list-tab').forEach(b => b.classList.remove('active'));
}

export function toggleMobileTopbar() {
  const topbar = document.getElementById('topbar');
  const mbtn = document.getElementById('mobile-topbar-toggle');
  const backdrop = document.getElementById('mobile-backdrop');
...
// ── Corporations panel ───────────────────────────────────────────────────────
// (corpCityQid moved to state.js)
// (corpCityName moved to state.js)
// (corpOverrideList moved to state.js)

// (fmtEmployees, fmtRevenue → utils.js)
...
// (_drawActive moved to state.js)
// (_drawVertices moved to state.js)
// (_drawPolyline moved to state.js)
// (_drawPolygon moved to state.js)
// (_drawDots moved to state.js)
// (_drawClickTimer moved to state.js)
...
function _onDrawContainerClick(e) {
  ...
  if (S._drawClickTimer) { clearTimeout(S._drawClickTimer); S._drawClickTimer = null; }
  const latlng = S.map.containerPointToLatLng(S.map.mouseEventToContainerPoint(e));
  S._drawClickTimer = setTimeout(() => { S._drawClickTimer = null; _drawAddVertex(latlng); }, 220);
}
...
function toggleDrawMode() {
  if (S._drawPolygon) { _drawClear(); return; }   // polygon exists → clear it
  if (S._drawActive)  { _drawFinish(); return; }  // drawing in progress → finish
  // Start drawing
  S._drawActive = true;
  S._drawVertices = [];
  S.map.dragging.disable();        // prevent accidental panning while placing vertices
  S.map.doubleClickZoom.disable(); // prevent zoom on double-click finish
  ...
  if (btn) { btn.innerHTML = '✓<span class="fab-label">Done</span>'; btn.title = 'Click to add points · Double-click or Done to close · Esc to cancel'; btn.classList.add('active'); }
...
function _drawClear() {
  ...
  S._drawActive = false;
  S._drawVertices = [];
  [S._drawPolyline, S._drawPolygon, ...S._drawDots].forEach(l => { try { if(l) S.map.removeLayer(l); } catch(_){} });
  ...
  const btn = document.getElementById('draw-fab');
  if (btn) { btn.innerHTML = '⬡'; btn.title = 'Draw a region to explore'; btn.classList.remove('active'); }
...
function _drawAddVertex(latlng) {
  ...
  const btn = document.getElementById('draw-fab');
  if (btn && S._drawVertices.length === 1) btn.title = 'Keep clicking to add points · Double-click or click Done to close';
}

function _drawFinish() {
  if (S._drawVertices.length < 3) { _drawClear(); return; }
  if (S._drawPolyline) { S.map.removeLayer(S._drawPolyline); S._drawPolyline = null; }
  ...
  S.map.getContainer().style.cursor = '';
  const btn = document.getElementById('draw-fab');
  if (btn) { btn.innerHTML = '✕<span class="fab-label">Clear</span>'; btn.title = 'Clear drawn region'; btn.classList.add('active'); }
  _drawProcessResults();
}
...
function _drawProcessResults() {
  const enclosed = S.allCities.filter(c =>
    c.lat != null && c.lng != null && _pointInPolygon(c.lat, c.lng, S._drawVertices)
  );
  if (!enclosed.length) {
...
function renderCorpList() {
  ...
  tbody.innerHTML = companies.map(co => {
    ...
    const revYearHtml = co.revenue_year
      ? ` <span style="color:var(--text-muted);font-size:0.7rem">${co.revenue_year}${histLen > 1 ? ` <span title="${histLen} years of data">·${histLen}yr</span>` : ''}</span>` : '';
...
// ── Global corporations list: extracted to src/corporations-list.js ────────────

// ── Reusable interactive canvas chart: extracted to src/iy-chart.js ────────────
const _IYChart = IYChart;
...
async function _renderCompanyPriceSection(ticker, containerEl) {
  ...
  try {
    ...
    const divHtml    = recentDivs.length
      ? `<div style="font-size:0.67rem;color:var(--text-secondary);padding:0 14px;margin-bottom:4px">Div (12mo): ${recentDivs.map(d => currSym + d.amount).join(' · ')}</div>`
      : '';
    ...
    ph.innerHTML = `
      ...
      ${divHtml}
      <div style="font-size:0.6rem;color:var(--text-muted);padding:0 14px 8px">${ticker} · ${data.exchange||''} · 5Y daily · ${data.updated||''}</div>
    `;
...
function _renderFinanceTab(co, containerEl) {
  ...
  const sec = label =>
    `<div style="padding:8px 14px 4px;font-size:0.65rem;font-weight:600;color:var(--text-faint);text-transform:uppercase;letter-spacing:0.06em;border-top:1px solid var(--bg-elevated)">${label}</div>`;
  ...
  // Price chart skeleton placeholder (no user data — safe static HTML)
  let html = '<div id="' + phId + '"><div class="skeleton-block">'
...
async function openCompanyWikiPanel(articleTitle, name, wikiUrl, finData = {}) {
  ...
  // Skeleton loading placeholders (no user data — safe static HTML)
  if (_coInfoEl) _coInfoEl.innerHTML = '<div class="skeleton-block">'
  ...
  // Lazy-load full company detail data if not yet loaded
  ...
  try {
    ...
    // Merge detail data into finData if available
    ...
    const _chipGrid = chips => chips
      ? `<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:5px;padding:10px 14px 8px;border-bottom:1px solid var(--bg-elevated)">${chips}</div>`
      : '';
    const finHtml =
      _chipGrid(keyChips) +
      _chipGrid(profileChips) +
      (trendRows ? `<div style="padding-bottom:6px;border-bottom:1px solid var(--bg-elevated)">${trendRows}</div>` : '');

    // Language badge shown when displaying non-English content
    const langBadge = displayLang !== 'en'
      ? `<span style="font-size:0.68rem;background:var(--bg-elevated);color:var(--text-secondary);border-radius:4px;padding:2px 6px;margin-left:8px;vertical-align:middle">${displayLang}</span>`
      : '';
...
// (_cmpCityA moved to state.js)
// (_cmpCityB moved to state.js)
...
function closeComparePanel() {
  ...
  _mobileBackdropOff();
  S._cmpCityA = null; S._cmpCityB = null;
}
...
(function initCitySearch() {
  ...
  function search(q) {
    ...
    if (!hits.length && !countryHits.length) { close(); return; }

    _selIdx = -1;
    const cityItems = hits.map(({ c }, i) =>
    ...
    const countryItems = countryHits.slice(0, 4).map((ch, i) =>
      `<li data-idx="${hits.length + i}" data-iso2="${escAttr(ch.iso)}"
           data-name="${escAttr(ch.name)}" class="csr-country">
         <span class="csr-name">🏳 ${highlight(ch.name, q)}</span>
         <span class="csr-meta">${ch.iso} · country</span>
       </li>`
    ).join('');
...
function toggleBookmark(qid) {
  ...
  _saveBookmarks();
  // Update the star button in the sidebar if visible
...
// (_ccIsoA moved to state.js)
// (_ccIsoB moved to state.js)
...
(function initCountryCompareSearch() {
  ...
  function search(q) {
    ...
    hits.slice(0, 8).forEach(h => {
      ...
      li.addEventListener('mousedown', e => { e.preventDefault(); selectCountry(h.iso); });
      results.appendChild(li);
    });
  }
...
export {
  ...
  renderCorpList, resetAll, saveEdit, setCensusColorMetric,
  ...
  togglePeeringdbLayer, toggleProtectedAreasLayer, toggleSatelliteLayer,
  toggleUnescoIchLayer, toggleVesselPortsLayer, toggleWaqiLayer, toggleWeatherLayer,
  toggleFlightAwareLayer, toggleMarineTrafficLayer, toggleEonetLayer,
  ...
  toggleKeyboardHelp,
  // Stat defs and helpers for stats-panel module
