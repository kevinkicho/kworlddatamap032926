...
// (_activeStatMetric moved to state.js)
// (_statsScope moved to state.js)
// (_statsCurrent moved to state.js)
// (_statsPoints moved to state.js)
// (_statsWinStart moved to state.js)
// (_statsWinEnd moved to state.js)
...
function _renderStatsPanel() {
  ...
  if (isWbStat) {
    ...
    if (wbDef.src === 'oecd') {
      // OECD data lives in oecdData, not countryData
...
function _initKeyboardNav() {
  document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      case 'Escape': {
        // Close topmost panel (highest z-index first)
        const panels = [
          ...
          { el: 'country-panel', check: 'open', close: () => { document.getElementById('country-panel').classList.remove('open'); S._cpCurrentIso2 = null; } },
...
function _toggleKeyboardHelp() {
  ...
  // Static content only — no user data
...
function buildUnescoLayer() {
  ...
  var markers = S.unescoSites.map(function(s) {
    ...
    marker.on('click', function() { openCountryPanel(s.iso2); });
    return marker;
  });
...
function buildCableLayer() {
  ...
  // Draw landing points (small dots, only at higher zoom)
...
function _greatCirclePoints(lat1, lng1, lat2, lng2, n) {
  ...
  // Returns { pts: [...], crossesAntimeridian: boolean }
...
function buildAirRouteLayer() {
  ...
  for (var i = 0; i < S.airRouteData.routes.length; i++) {
    ...
    // If route crosses antimeridian, split into two polylines
    if (result.crossesAnti) {
      // Find the point where route crosses antimeridian
      var splitIdx = -1;
...
function _quakeColor(depth) {
  if (depth < 70) return '#ffd166';    // shallow — yellow
  if (depth < 300) return '#ef8354';   // intermediate — orange
...
function _volcanoColor(lastEruption) {
  if (lastEruption == null) return '#888';        // unknown — gray
  if (lastEruption > 1900) return '#d62828';      // active — red
  if (lastEruption > 0) return '#ef8354';         // historical — orange
  if (lastEruption > -10000) return '#ffd166';   // holocene — yellow
  return '#888';                                    // pleistocene — gray
}
...
// Helper for dropdown button text updates
function _setLayerBtn(id, emoji, label, isOn) {
...
function _buildGtdLayer() {
  ...
  for (const inc of S.gtdData.incidents || []) {
    ...
    const radius = Math.max(6, Math.min(14, inc.fatalities / 25)); // Larger radius for visibility
...
function _buildCryptoLayer() {
  ...
  // Color scale based on adoption rank (lower rank = higher adoption = green)
  const getAdoptionColor = (rank) => {
    if (rank <= 5) return '#22c55e';      // Top 5: Green
    if (rank <= 10) return '#84cc16';     // Top 10: Light green
    if (rank <= 20) return '#eab308';     // Top 20: Yellow
    if (rank <= 35) return '#f97316';     // Top 35: Orange
    return '#ef4444';                      // Rest: Red
  };
...
function _buildOceanLayer() {
  ...
  // Speed-based color scale (cm/s)
  const getSpeedColor = (speed) => {
    if (speed >= 150) return '#ef4444';      // Very fast (red)
    if (speed >= 100) return '#f97316';      // Fast (orange)
    if (speed >= 70) return '#eab308';       // Moderate (yellow)
    if (speed >= 40) return '#22c55e';       // Slow (green)
    return '#3b82f6';                         // Very slow (blue)
  };
  ...
  for (const c of S.oceanData.currents || []) {
    ...
    // Add arrowhead using a marker with rotation
    ...
    // Add a subtle glow effect for fast currents
    if (c.spd >= 100) {
...
// (_expandedLayers moved to state.js)
// (_collapseOnClick moved to state.js)
// (_pinnedExpansion moved to state.js)
...
function expandEconCluster(group, clusterUSD, cLat, cLng) {
  ...
  S._pinnedExpansion = { group, clusterUSD, cLat, cLng }; // pin so zoom rebuilds re-expand
  const layers = [];
...
function buildEconLayer() {
  if (!S.econOn) return;
  const _savedPin = S._pinnedExpansion;  // preserve across rebuild so zoom keeps expansion
  _clearExpandedLayers();              // remove old expansion visuals (don't clear pin yet)
  S._pinnedExpansion = null;             // reset so re-expand below is clean
  if (S.econLayer) { S.map.removeLayer(S.econLayer); S.econLayer = null; }
  ...
  for (const group of clusters) {
    ...
    const m = L.circleMarker([lat, lng], {
      ...
      opacity: 0.9,
      pane: 'econPane',
      bubblingMouseEvents: false,    // prevent click from bubbling to S.map collapse handler
    });
...
// (globalCorpList moved to state.js)
// (globalCorpVis moved to state.js)
const GCORP_PAGE = 100;
// (gcorpQuery moved to state.js)
// (gcorpCountry moved to state.js)
// (gcorpIndustry moved to state.js)
// (gcorpSort moved to state.js)
function buildGlobalCorpList() {
  ...
  for (const [qid, companies] of Object.entries(S.companiesData)) {
    ...
    for (const co of companies) {
      ...
      // If the tagged currency gave an implausible result (> $5T) and we have a fallback, retry with it
...
function gcorpQueryChanged(v) { S.gcorpQuery = v; S.globalCorpVis = GCORP_PAGE; renderGlobalCorpList(); }
...
// Count dropdown open/close handlers
document.addEventListener('click', (e) => {
...
function gcorpCountryChanged(v) { S.gcorpCountry = v; S.globalCorpVis = GCORP_PAGE; renderGlobalCorpList(); }
function gcorpIndustryChanged(v) { S.gcorpIndustry = v; S.globalCorpVis = GCORP_PAGE; renderGlobalCorpList(); }
function gcorpSortChanged(v) { S.gcorpSort = v; S.globalCorpVis = GCORP_PAGE; renderGlobalCorpList(); }
