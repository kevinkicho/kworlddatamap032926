...
// Pre-built QID → FIPS map (generated by scripts/fetch-census-fips.js, no CORS issues)
let _censusFipsMap = null;
async function _loadCensusFipsMap() {
  if (_censusFipsMap) return _censusFipsMap;
  try {
    const res = await fetch('census-fips.json');
    _censusFipsMap = res.ok ? await res.json() : {};
  } catch (_) { _censusFipsMap = {}; }
  return _censusFipsMap;
}
...
async function fetchCensusFips(city) {
  const map = await _loadCensusFipsMap();
  return map[city.qid] || null;   // null means not found (metro areas, unincorporated places, etc.)
}
